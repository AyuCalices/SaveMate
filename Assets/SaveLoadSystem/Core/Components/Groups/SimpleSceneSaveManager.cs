...
using SaveLoadSystem.Core.Components.Groups.Interface;
        ...
        private BranchSaveData CreateBranchSaveData(SaveFileContext saveFileContext, SaveLoadManager saveLoadManager)
        {
            ...
            foreach (var saveObject in _trackedSavables.Values)
            {
                ...
                foreach (var componentContainer in saveObject.SaveStateHandlers)
                {
                    ...
                    iSavable.OnCaptureState(new CreateSnapshotHandler(branchSaveData, leafSaveData, guidPath, SceneName, saveFileContext, saveLoadManager));
